								  ...
								  unsigned long * pBufferSize)
{
	...
	if (!IsListEmpty(&pAddr->pendedSendRequests))
	{
		...
		if (irpSp->MinorFunction == TDI_SEND_DATAGRAM)
		{
			...
			if (prk)
			{
				KdPrint((DPREFIX"ctrl_copyUdpSendToBuffer[%I64d] UserDataLength=%d, UserData=%p\n", 
					pEntry->id,
					prk->SendDatagramInformation->UserDataLength,
					prk->SendDatagramInformation->UserData));
