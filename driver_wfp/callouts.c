...
VOID callouts_connectAuthCallout(
   IN const FWPS_INCOMING_VALUES* inFixedValues,
   IN const FWPS_INCOMING_METADATA_VALUES* inMetaValues,
   IN VOID* packet,
   IN const void* classifyContext,
...
NTSTATUS callouts_connectAuthNotify(
    IN  FWPS_CALLOUT_NOTIFY_TYPE        notifyType,
    IN  const GUID*             filterKey,
    IN  const FWPS_FILTER*     filter);

VOID callouts_connectAuthDiscardCallout(
   IN const FWPS_INCOMING_VALUES* inFixedValues,
   IN const FWPS_INCOMING_METADATA_VALUES* inMetaValues,
   IN VOID* packet,
   IN const void* classifyContext,
...
NTSTATUS callouts_connectAuthDiscardNotify(
    IN  FWPS_CALLOUT_NOTIFY_TYPE        notifyType,
    IN  const GUID*             filterKey,
    IN  const FWPS_FILTER*     filter);


ULONG callouts_getSidLength(PISID pSid)
{
	return sizeof(SID) + sizeof(ULONG) * (pSid->SubAuthorityCount - 1);
}
...
callouts_unregisterCallouts()
{
	...
	for (i=0; i<CG_MAX; i++)
	{
		status = pFwpsCalloutUnregisterByKey0(&g_calloutGuids[i]);
...
struct NF_CALLOUT
{
   ...
   UINT32* calloutId;
	...
	{
        (FWPS_CALLOUT_CLASSIFY_FN)callouts_connectAuthCallout,
        (FWPS_CALLOUT_NOTIFY_FN)callouts_connectAuthNotify, 
        NULL,
        &g_calloutGuids[CG_ALE_CONNECT_AUTH_V4],
        0, // No flags.
        &g_calloutIds[CG_ALE_CONNECT_AUTH_V4]
	},
	{
        (FWPS_CALLOUT_CLASSIFY_FN)callouts_connectAuthCallout,
        (FWPS_CALLOUT_NOTIFY_FN)callouts_connectAuthNotify,
        NULL, // We don't need a flow delete function at this layer.
        &g_calloutGuids[CG_ALE_CONNECT_AUTH_V6],
        0, // No flags.
        &g_calloutIds[CG_ALE_CONNECT_AUTH_V6]
	},
	{
        (FWPS_CALLOUT_CLASSIFY_FN)callouts_connectAuthDiscardCallout,
        (FWPS_CALLOUT_NOTIFY_FN)callouts_connectAuthDiscardNotify, 
        NULL,
        &g_calloutGuids[CG_ALE_CONNECT_AUTH_V4_DISCARD],
        0, // No flags.
        &g_calloutIds[CG_ALE_CONNECT_AUTH_V4_DISCARD]
	},
	{
        (FWPS_CALLOUT_CLASSIFY_FN)callouts_connectAuthDiscardCallout,
        (FWPS_CALLOUT_NOTIFY_FN)callouts_connectAuthDiscardNotify,
        NULL, // We don't need a flow delete function at this layer.
        &g_calloutGuids[CG_ALE_CONNECT_AUTH_V6_DISCARD],
        0, // No flags.
        &g_calloutIds[CG_ALE_CONNECT_AUTH_V6_DISCARD]
	},
   ...
   )
{
    ...
    status = pFwpsCalloutRegister1(deviceObject, (FWPS_CALLOUT1*)&sCallout, calloutId);
   ...
   )
{
	...
	status = pFwpmTransactionBegin0(g_engineHandle, 0);
	if (!NT_SUCCESS(status))
	{
		pFwpmEngineClose0(g_engineHandle);
        g_engineHandle = NULL;
		return status;
	}
	...
	for(;;)
	{
		for (i=0; i<sizeof(g_callouts)/sizeof(g_callouts[0]); i++)
		{
			...
			if (!NT_SUCCESS(status))
			{
				KdPrint((DPREFIX"callouts_registerCallouts: error=%x\n", status));
				break;
			}
		}
		...
		status = pFwpmTransactionCommit0(g_engineHandle);
		if (!NT_SUCCESS(status))
		{
			break;
		}
	...
	{
		KdPrint((DPREFIX"callouts_registerCallouts: error=%x\n", status));
		pFwpmTransactionAbort0(g_engineHandle);
		pFwpmEngineClose0(g_engineHandle);
        g_engineHandle = NULL;
	}
...
callouts_addFlowEstablishedFilter(const GUID * calloutKey, const GUID * layer, FWPM_SUBLAYER * subLayer)
{
	...
	for (;;)
	{
		...
		status = pFwpmCalloutAdd0(g_engineHandle, &callout, NULL, NULL);
		if (!NT_SUCCESS(status))
		{
			break;
		}
		...
		status = pFwpmFilterAdd0(g_engineHandle,
						   &filter,
						   NULL,
						   NULL);
...
//	DbgBreakPoint();
	...
	if (pFwpmSubLayerCreateEnumHandle0(g_engineHandle, NULL, &hEnum) != 0)
		return minWeight;
	...
	for (;;)
	{
		...
		if (pFwpmSubLayerEnum0(g_engineHandle, hEnum, 1, &pSublayers, &nItems) != 0)
			break;
		if (nItems < 1)
			break;
	...
	pFwpmSubLayerDestroyEnumHandle0(g_engineHandle, hEnum);
...
callouts_addFilters()
{
	...
	if (callouts_isDriverRegistered(L"bdfwfpf_pc") ||
		callouts_isDriverRegistered(L"bdfwfpf")) 
	{
...
//		g_bypassConnectRedirectWithoutActionWrite = FALSE;
	...
	for (;;)
	{
		...
		status = pFwpmSubLayerAdd0(g_engineHandle, &subLayer, NULL);
		if (!NT_SUCCESS(status))
		{
			break;
		}
		...
		status = pFwpmSubLayerAdd0(g_engineHandle, &recvSubLayer, NULL);
		if (!NT_SUCCESS(status))
		{
			break;
		}
		...
		status = pFwpmSubLayerAdd0(g_engineHandle, &recvProtSubLayer, NULL);
		if (!NT_SUCCESS(status))
		{
			break;
		}
		...
		status = pFwpmSubLayerAdd0(g_engineHandle, &ipSubLayer, NULL);
		if (!NT_SUCCESS(status))
		{
			break;
		}
		...
		if (!NT_SUCCESS(status))
		{
			KdPrint((DPREFIX"callouts_addFilters: transaction commit error=%x\n", status));
			break;
		}
	...
	if (!NT_SUCCESS(status))
	{
		KdPrint((DPREFIX"callouts_addFilters: transaction abort, error=%x\n", status));
		pFwpmTransactionAbort0(g_engineHandle);
	}
   ...
   IN const FWPS_INCOMING_METADATA_VALUES* inMetaValues)
{
	...
	pTcpCtx->connectRequestTS = flowctl_getTickCount();
	...
	if (inFixedValues->layerId == FWPS_LAYER_ALE_FLOW_ESTABLISHED_V4)
	{
		...
		if (wfplink_checkMinimumWindowsVersion(8))
		{
			PISID pSid = inFixedValues->incomingValue[FWPS_FIELD_ALE_FLOW_ESTABLISHED_V4_ALE_PACKAGE_ID].value.sid;
			if (pSid)
			{
				ULONG sidLength;

				sidLength = callouts_getSidLength(pSid);
				pTcpCtx->pPackageSid = (PISID)malloc_np(sidLength);
				if (pTcpCtx->pPackageSid)
				{
					memcpy(pTcpCtx->pPackageSid, pSid, sidLength);
				}
			}
		}
	...
	if (inFixedValues->layerId == FWPS_LAYER_ALE_FLOW_ESTABLISHED_V6)
	{
		...
		if (wfplink_checkMinimumWindowsVersion(8))
		{
			PISID pSid = inFixedValues->incomingValue[FWPS_FIELD_ALE_FLOW_ESTABLISHED_V6_ALE_PACKAGE_ID].value.sid;
			if (pSid)
			{
				ULONG sidLength;
	...
	if (inFixedValues->layerId == FWPS_LAYER_ALE_CONNECT_REDIRECT_V4)
	{
		...
		if (wfplink_checkMinimumWindowsVersion(8))
		{
			PISID pSid = inFixedValues->incomingValue[FWPS_FIELD_ALE_CONNECT_REDIRECT_V4_ALE_PACKAGE_ID].value.sid;
			if (pSid)
			{
				ULONG sidLength;
	...
	if (inFixedValues->layerId == FWPS_LAYER_ALE_CONNECT_REDIRECT_V6)
	{
		...
		if (wfplink_checkMinimumWindowsVersion(8))
		{
			PISID pSid = inFixedValues->incomingValue[FWPS_FIELD_ALE_CONNECT_REDIRECT_V6_ALE_PACKAGE_ID].value.sid;
			if (pSid)
			{
				ULONG sidLength;
	...
	if (inFixedValues->layerId == FWPS_LAYER_ALE_AUTH_CONNECT_V4)
	{
		...
		pTcpCtx->ipProto = inFixedValues->incomingValue[FWPS_FIELD_ALE_AUTH_CONNECT_V4_IP_PROTOCOL].value.uint8;
		pTcpCtx->ip_family = AF_INET;
		...
		if (inFixedValues->incomingValue[FWPS_FIELD_ALE_AUTH_CONNECT_V4_ALE_APP_ID].value.byteBlob)
		{
			int offset, len;
	
			len = inFixedValues->incomingValue[FWPS_FIELD_ALE_AUTH_CONNECT_V4_ALE_APP_ID].value.byteBlob->size;
			if (len > sizeof(pTcpCtx->processName) - 2)
			{
				offset = len - (sizeof(pTcpCtx->processName) - 2);
				len = sizeof(pTcpCtx->processName) - 2;
			...
			memcpy(pTcpCtx->processName, 
				inFixedValues->incomingValue[FWPS_FIELD_ALE_AUTH_CONNECT_V4_ALE_APP_ID].value.byteBlob->data + offset,
				len);
		}
		...
		if (wfplink_checkMinimumWindowsVersion(8))
		{
			PISID pSid = inFixedValues->incomingValue[FWPS_FIELD_ALE_AUTH_CONNECT_V4_ALE_PACKAGE_ID].value.sid;
			if (pSid)
			{
				ULONG sidLength;
	...
	if (inFixedValues->layerId == FWPS_LAYER_ALE_AUTH_CONNECT_V6)
	{
		...
		pTcpCtx->ipProto = inFixedValues->incomingValue[FWPS_FIELD_ALE_AUTH_CONNECT_V6_IP_PROTOCOL].value.uint8;
		pTcpCtx->ip_family = AF_INET6;

		pAddr = (struct sockaddr_in6*)pTcpCtx->localAddr;
		pAddr->sin6_family = AF_INET6;
		memcpy(&pAddr->sin6_addr, 
				inFixedValues->incomingValue[FWPS_FIELD_ALE_AUTH_CONNECT_V6_IP_LOCAL_ADDRESS].value.byteArray16->byteArray16,
				NF_MAX_IP_ADDRESS_LENGTH);
		pAddr->sin6_port = 
			htons(inFixedValues->incomingValue[FWPS_FIELD_ALE_AUTH_CONNECT_V6_IP_LOCAL_PORT].value.uint16);
		...
		pAddr->sin6_family = AF_INET6;
		memcpy(&pAddr->sin6_addr, 
				inFixedValues->incomingValue[FWPS_FIELD_ALE_AUTH_CONNECT_V6_IP_REMOTE_ADDRESS].value.byteArray16->byteArray16,
				NF_MAX_IP_ADDRESS_LENGTH);
		pAddr->sin6_port = 
			htons(inFixedValues->incomingValue[FWPS_FIELD_ALE_AUTH_CONNECT_V6_IP_REMOTE_PORT].value.uint16);
		...
		if (inFixedValues->incomingValue[FWPS_FIELD_ALE_AUTH_CONNECT_V6_ALE_APP_ID].value.byteBlob)
		{
			int offset, len;
	
			len = inFixedValues->incomingValue[FWPS_FIELD_ALE_AUTH_CONNECT_V6_ALE_APP_ID].value.byteBlob->size;
			if (len > sizeof(pTcpCtx->processName) - 2)
			{
				offset = len - (sizeof(pTcpCtx->processName) - 2);
				len = sizeof(pTcpCtx->processName) - 2;
			...
			memcpy(pTcpCtx->processName, 
				inFixedValues->incomingValue[FWPS_FIELD_ALE_AUTH_CONNECT_V6_ALE_APP_ID].value.byteBlob->data + offset,
				len);
		}
		...
		if (wfplink_checkMinimumWindowsVersion(8))
		{
			PISID pSid = inFixedValues->incomingValue[FWPS_FIELD_ALE_AUTH_CONNECT_V6_ALE_PACKAGE_ID].value.sid;
			if (pSid)
			{
				ULONG sidLength;
   ...
   OUT FWPS_CLASSIFY_OUT* classifyOut)
{
	...
	for (;;) 
	{
		pTcpCtxRd = tcpctx_findByHandle(inMetaValues->transportEndpointHandle);
		if (pTcpCtxRd)
		{
			tcpctx_removeFromHandleTable(pTcpCtxRd);
			pTcpCtxRd->transportEndpointHandle = 0;

			pTcpCtx = callouts_createFlowContext(pTcpCtxRd, inFixedValues, inMetaValues);
			
			tcpctx_release(pTcpCtxRd);
...
//			break;
		...
		status = pFwpsFlowAssociateContext0(pTcpCtx->flowHandle,
                                     pTcpCtx->layerId,
	...
	if (inFixedValues->layerId == FWPS_LAYER_ALE_FLOW_ESTABLISHED_V4)
	{
		...
		if (wfplink_checkMinimumWindowsVersion(8))
		{
			...
			if (pSid)
			{
				...
				sidLength = callouts_getSidLength(pSid);
				pUdpCtx->pPackageSid = (PISID)malloc_np(sidLength);
				if (pUdpCtx->pPackageSid)
				{
					memcpy(pUdpCtx->pPackageSid, pSid, sidLength);
				}
			}
		}
#endif // (NTDDI_VERSION >= NTDDI_WIN8)
   ...
   OUT FWPS_CLASSIFY_OUT* classifyOut)
{
	...
	if (isSend && (devctrl_udpNwInject() || !inMetaValues->transportEndpointHandle))
	{
		packetState = pFwpsQueryPacketInjectionState0(
	...
	if (!flowContext)
	{
		...
		if (pUdpCtx)
		{
			...
			if (inFixedValues->layerId == FWPS_LAYER_DATAGRAM_DATA_V6)
			{
				...
				if (_memcmp(&pAddr->sin6_addr, zero, 16) == 0)
				{
					pUdpCtx->layerId = FWPS_LAYER_DATAGRAM_DATA_V6;
					pUdpCtx->calloutId = g_calloutIds[CG_DATAGRAM_DATA_CALLOUT_V6];
					pUdpCtx->ipProto = inFixedValues->incomingValue[FWPS_FIELD_DATAGRAM_DATA_V6_IP_PROTOCOL].value.uint8;
...
*/
		...
		if (mustClose)
		{
			tcpctx_removeContextFlows(pTcpCtx);
		}
	}
   ...
   OUT FWPS_CLASSIFY_OUT* classifyOut)
{
	...
	for (;;) 
	{
		...
		pTcpCtx->filteringFlag = rules_findByTcpCtx(pTcpCtx, &matchingRule);
		...
		if (pTcpCtx->filteringFlag & NF_REDIRECT)
		{
			FWPS_CONNECT_REQUEST * pConnectRequest;
			int addrLen = 0;

			KdPrint((DPREFIX"callouts_connectRedirectCallout[%I64u] redirect endpoint %I64u\n", 
			...
			if (wfplink_checkMinimumWindowsVersion(8))
			{
				status = pFwpsRedirectHandleCreate0(&g_providerGuid, 0, &pTcpCtx->redirectInfo.redirectHandle);
				if (status != STATUS_SUCCESS)
				{
					classifyOut->actionType = FWP_ACTION_PERMIT;
...
#endif
#endif
			status = pFwpsAcquireClassifyHandle0((void*)classifyContext,
											  0,
			...
			if ((_memcmp(pTcpCtx->remoteAddr, matchingRule.redirectTo, addrLen) != 0) ||
				(pTcpCtx->filteringFlag & NF_BLOCK))
			{
				KdPrint((DPREFIX"callouts_connectRedirectCallout[%I64u]: redirection\n", pTcpCtx->id));
		
				status = pFwpsAcquireWritableLayerDataPointer0(pTcpCtx->redirectInfo.classifyHandle,
				...
				if (status == STATUS_SUCCESS)
				{
					...
					{
						memcpy(&pConnectRequest->remoteAddressAndPort, matchingRule.redirectTo, NF_MAX_ADDRESS_LENGTH);

	#ifdef USE_NTDDI
	#if (NTDDI_VERSION >= NTDDI_WIN8)
						if (wfplink_checkMinimumWindowsVersion(8))
						{
							pConnectRequest->localRedirectContext = malloc_np(NF_MAX_ADDRESS_LENGTH);
							if (pConnectRequest->localRedirectContext)
							{
								memcpy(pConnectRequest->localRedirectContext, pTcpCtx->remoteAddr, NF_MAX_ADDRESS_LENGTH);
								pConnectRequest->localRedirectContextSize = NF_MAX_ADDRESS_LENGTH;
							}
						}
	#endif
	#endif
					...
					dbgPrintAddress(pTcpCtx->ip_family, &pConnectRequest->remoteAddressAndPort, "remoteAddr", pTcpCtx->id);
		
					pConnectRequest->localRedirectTargetPID = matchingRule.localProxyProcessId;

					KdPrint((DPREFIX"callouts_connectRedirectCallout[%I64u]: processId %d\n", pTcpCtx->id, matchingRule.localProxyProcessId));
					...
					pFwpsApplyModifiedLayerData0(pTcpCtx->redirectInfo.classifyHandle,
											 pConnectRequest,
											 0);
				} else
				{
					classifyOut->actionType = FWP_ACTION_PERMIT;
					KdPrint((DPREFIX"callouts_connectRedirectCallout FwpsAcquireWritableLayerDataPointer failed, status=%x\n", status));
					break;
				}
			}
		...
		{
			PTCPCTX pTcpCtxOld = NULL;

			memcpy(&pTcpCtx->redirectInfo.classifyOut, classifyOut, sizeof(FWPS_CLASSIFY_OUT));

			pTcpCtxOld = tcpctx_findByHandle(inMetaValues->transportEndpointHandle);
			if (pTcpCtxOld)
			{
				KdPrint((DPREFIX"callouts_connectRedirectCallout[%I64u] found another context id=%I64u with the same handle\n", 
					pTcpCtx->id, pTcpCtxOld->id));

				tcpctx_release(pTcpCtxOld);
			...
			status = pFwpsPendClassify0(pTcpCtx->redirectInfo.classifyHandle,
									 filter->filterId,
									 0,
									 &pTcpCtx->redirectInfo.classifyOut);
			if (status != STATUS_SUCCESS)
			...
			pTcpCtx->connectRequestPended = TRUE;
   ...
   OUT FWPS_CLASSIFY_OUT* classifyOut)
{
	...
	} else
	{
		...
		if (_memcmp(inFixedValues->incomingValue[FWPS_FIELD_DATAGRAM_DATA_V6_IP_REMOTE_ADDRESS].value.byteArray16->byteArray16, zero, 16) != 0)
		{
			KdPrint((DPREFIX"callouts_connectRedirectUdpCallout bypass generic send\n"));
			classifyOut->actionType = FWP_ACTION_PERMIT;
			return;
	...
	for (;;) 
	{
		...
		if (pUdpCtx->filteringFlag & NF_INDICATE_CONNECT_REQUESTS)
		{
			...
			if (wfplink_checkMinimumWindowsVersion(8))
			{
				status = pFwpsRedirectHandleCreate0(&g_providerGuid, 0, &pUdpCtx->redirectInfo.redirectHandle);
				if (status != STATUS_SUCCESS)
				{
					classifyOut->actionType = FWP_ACTION_PERMIT;
			...
			status = pFwpsPendClassify0(pUdpCtx->redirectInfo.classifyHandle,
									 filter->filterId,
									 0,
	...
	for (;;) 
	{
		...
		if (inFixedValues->layerId == FWPS_LAYER_ALE_BIND_REDIRECT_V4)
		{
			...
			*(ULONG*)bindInfo.localIpAddress = 
			...
			if (wfplink_checkMinimumWindowsVersion(8))
			{
				PISID pSid = inFixedValues->incomingValue[FWPS_FIELD_ALE_BIND_REDIRECT_V4_ALE_PACKAGE_ID].value.sid;
				if (pSid)
				{
					pPackageSid = pSid;
				}
		...
		if (inFixedValues->layerId == FWPS_LAYER_ALE_BIND_REDIRECT_V6)
		{
			...
			if (wfplink_checkMinimumWindowsVersion(8))
			{
				PISID pSid = inFixedValues->incomingValue[FWPS_FIELD_ALE_BIND_REDIRECT_V6_ALE_PACKAGE_ID].value.sid;
				if (pSid)
				{
					pPackageSid = pSid;
				}
		...
		filteringFlag = rules_findByBindInfo(&bindInfo, pPackageSid);
		...
		if (filteringFlag & NF_FILTER)
		{
			...
			status = pFwpsAcquireWritableLayerDataPointer0(classifyHandle,
														filter->filterId,
														0,
														(PVOID*)&pBindRequest,
														classifyOut);
			if (status == STATUS_SUCCESS)
			{
				...
				pFwpsApplyModifiedLayerData0(classifyHandle,
										 pBindRequest,
										 0);
			} else
			{
		    ...
		    pFwpsReleaseClassifyHandle0(classifyHandle);
   ...
   OUT FWPS_CLASSIFY_OUT* classifyOut)
{
	...
	if ((classifyOut->rights & FWPS_RIGHT_ACTION_WRITE) == 0)
	{
		KdPrint((DPREFIX"callouts_connectAuthCallout no FWPS_RIGHT_ACTION_WRITE, actionType=%d\n", classifyOut->actionType));
		return;
	}
	...
	for (;;) 
	{
		PTCPCTX pTcpCtxRd = NULL;
		ULONG filteringFlag = 0;
		...
		if (pTcpCtxRd)
		{
			...
			filteringFlag = pTcpCtx->filteringFlag;
		} else
		{
			pTcpCtx = callouts_createFlowContext(NULL, inFixedValues, inMetaValues);
			if (!pTcpCtx)
			...
			filteringFlag = rules_findByTcpCtx(pTcpCtx, NULL);
		}

		KdPrint((DPREFIX"callouts_connectAuthCallout[%I64u] flags = %d\n", pTcpCtx->id, filteringFlag));

		if (filteringFlag & NF_BLOCK)
		{
			classifyOut->actionType = FWP_ACTION_BLOCK;
			classifyOut->rights ^= FWPS_RIGHT_ACTION_WRITE;
			KdPrint((DPREFIX"callouts_connectAuthCallout block\n"));
			break;
		}
    ...
    IN  const FWPS_FILTER*     filter)
{
    ...
    {
    case FWPS_CALLOUT_NOTIFY_ADD_FILTER:
        KdPrint((DPREFIX"Filter Added to connectAuth layer.\n"));
       break;
    case FWPS_CALLOUT_NOTIFY_DELETE_FILTER:
        KdPrint((DPREFIX"Filter Deleted from connectAuth layer.\n"));
       break;
    }
    return STATUS_SUCCESS;
}
   ...
   OUT FWPS_CLASSIFY_OUT* classifyOut)
{
	...
	KdPrint((DPREFIX"callouts_connectAuthDiscardCallout\n"));

	pTcpCtx = tcpctx_findByHandle(inMetaValues->transportEndpointHandle);
	if (pTcpCtx)
	{
		KdPrint((DPREFIX"callouts_connectAuthDiscardCallout[%I64u]\n", pTcpCtx->id));
    ...
    IN  const FWPS_FILTER*     filter)
{
    ...
    {
    case FWPS_CALLOUT_NOTIFY_ADD_FILTER:
        KdPrint((DPREFIX"Filter Added to connectAuthDiscard layer.\n"));
       break;
    case FWPS_CALLOUT_NOTIFY_DELETE_FILTER:
        KdPrint((DPREFIX"Filter Deleted from connectAuthDiscard layer.\n"));
       break;
    }
    return STATUS_SUCCESS;
}
...
NTSTATUS callouts_init(PDEVICE_OBJECT deviceObject)
{
	...
	for (;;)
	{
		...
		dwStatus = pFwpmProviderAdd0(g_engineHandle, &provider, NULL);
		if (dwStatus != 0)
		{
...
void callouts_free()
{
	...
	pFwpmSubLayerDeleteByKey0(g_engineHandle, &g_sublayerGuid);
	pFwpmSubLayerDeleteByKey0(g_engineHandle, &g_recvSublayerGuid);
	pFwpmSubLayerDeleteByKey0(g_engineHandle, &g_recvProtSublayerGuid);
	pFwpmSubLayerDeleteByKey0(g_engineHandle, &g_ipSublayerGuid);

	pFwpmProviderContextDeleteByKey0(g_engineHandle, &g_providerGuid);
