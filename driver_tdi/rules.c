...
void dbgPrintAddress(int ipFamily, void * addr, char * name, UINT64 id)
{
	if (ipFamily == AF_INET)
	{
		...
		KdPrint((DPREFIX"dbgPrintAddress[%I64u] %s=%x:%d\n", 
			id, name, pAddr->sin_addr.S_un.S_addr, ntohs(pAddr->sin_port)));
	} else
	{
...
NF_FILTERING_FLAG rule_findByConn(PCONNOBJ pConn)
{
	...
	localPortHostOrder = ntohs(pLocalAddress->AddressIn.sin_port);
	remotePortHostOrder = ntohs(pRemoteAddress->AddressIn.sin_port);

	dbgPrintAddress(AF_INET, pRemoteAddress, "TCP remoteAddr", pConn->id);
...
NF_FILTERING_FLAG rule_findByAddr(PADDROBJ pAddr, NF_DIRECTION direction, sockaddr_gen * remoteAddr)
{
	...
	dbgPrintAddress(AF_INET, pRemoteAddress, "UDP remoteAddr", pAddr->id);
