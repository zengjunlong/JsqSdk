...
#define MAX_BUFFERED_BYTES_IN		16 * NF_TCP_PACKET_BUF_SIZE

#define MAX_SEND_QUEUE_BYTES		32 * NF_TCP_PACKET_BUF_SIZE
#define MAX_QUEUE_BYTES				8 * 1024 * 1024
#define MAX_OUT_QUEUE_BYTES			1 * 1024 * 1024
...
typedef struct _CONNOBJ
{
	...
	bool			connectRequestCalled;
	DWORD			timeout;			// Connection timeout
} CONNOBJ, *PCONNOBJ;
...
static int					g_maxBufferedBytes = MAX_BUFFERED_BYTES;
static unsigned long		g_driverType = 0;
...
static bool largeBuffersSupported();
...
static NF_STATUS nf_postData(PNF_DATA pData)
{
	...
	if (!g_initialized || g_hDevice == INVALID_HANDLE_VALUE || !g_nfBuffers.outBuf)
...
*/
			...
			if (pData->bufferSize > 0)
			{
				if (!IsListEmpty(pPacketList))
				{
					...
					if ((pDataEntry->data.bufferSize + pData->bufferSize) <= MAX_BUFFERED_BYTES_IN)
					{
						if (pData->bufferSize > 0)
						{
			...
			if (!packetAdded)
			{
				pDataEntry = (PNF_DATA_ENTRY)mp_alloc(sizeof(NF_DATA_ENTRY) + MAX_BUFFERED_BYTES_IN - 1);
				if (!pDataEntry)
					break;
...
static void nf_closeHangingConnections()
{
	...
	while (pConn != (PCONNOBJ)&g_lConnections)
	{
		...
		{	
			...
			if (pConn->timeout && (dwTime > pConn->timeout))
			{
				nf_tcpClose(pConn->id);
		...
		if (!pConn->connected && !pConn->connectRequestCalled)
		{
			dwTime = dwCurrentTime - pConn->lastIoTime;
			
			if (dwTime > 2 * TCP_TIMEOUT_CHECK_PERIOD)
			{
				pConn->connectRequestCalled = true;
				...
				if (pData)
				{
					...
					memcpy(pData->buffer, &pConn->connInfo, sizeof(NF_TCP_CONN_INFO));
...
static NF_STATUS nf_tcpPostPacket(NF_DATA_CODE code, ENDPOINT_ID id, const char * buf, int len)
{
	...
	if (!g_initialized || g_hDevice == INVALID_HANDLE_VALUE)
		return NF_STATUS_NOT_INITIALIZED;
	...
	if (len <= g_maxBufferedBytes)
	{
		...
		if ((len > 0) && !IsListEmpty(pPacketList))
		{
			...
			if (pData->data.bufferSize > 0 &&
				(pData->data.bufferSize + len) <= (unsigned long)g_maxBufferedBytes)
			{
				if (buf)
				{
		...
		if (!packetAdded)
		{
			pData = (PNF_DATA_ENTRY)mp_alloc(sizeof(NF_DATA_ENTRY) + g_maxBufferedBytes - 1);
			if (!pData)
				return NF_STATUS_FAIL;
	...
	} else
	{
		...
		while (offset < len)
		{
			...
			if (packetLength > g_maxBufferedBytes)
				packetLength = g_maxBufferedBytes;
	...
	if (!IsListEmpty(pPacketList))// && ((code == NF_TCP_SEND) || !pConn->indicateReceiveInProgress))
	{
...
nf_setRulesEx(PNF_RULE_EX pRules, int count)
{
	...
	for (int i=0; i<count; i++)
	{
		if (pRules[i].processName[0] != 0)
		{
			size_t len = wcslen((wchar_t*)pRules[i].processName);

			wcslwr((wchar_t*)pRules[i].processName);
			if (len > 3)
			{
				if (pRules[i].processName[1] == L':')
				{
					memmove(pRules[i].processName, pRules[i].processName + 2, len*2 - 2);
				}
			}
		}
	}
...
NFAPI_API NF_STATUS NFAPI_NS nf_init(const char * driverName, NF_EventHandler * pHandler)
{
	...
	if (largeBuffersSupported())
	{
		g_maxBufferedBytes = 16 * NF_TCP_PACKET_BUF_SIZE;
	}

	g_driverType = nf_getDriverType();
...
NF_STATUS nf_getLocalAddress(ENDPOINT_ID id, unsigned char * addr, int len)
{
	...
	tQueryFullProcessImageNameW QueryFullProcessImageNameW =
				(tQueryFullProcessImageNameW)GetProcAddress(
								GetModuleHandleA("kernel32"), "QueryFullProcessImageNameW");

	tQueryFullProcessImageNameA QueryFullProcessImageNameA =
				(tQueryFullProcessImageNameA)GetProcAddress(
								GetModuleHandleA("kernel32"), "QueryFullProcessImageNameA");
#endif
...
nf_addBindingRule(PNF_BINDING_RULE pRule, int toHead)
{
	...
	if (pRule->processName[0] != 0)
	{
		...
		wcslwr((wchar_t*)pRule->processName);
		if (len > 3)
		{
...
largeBuffersSupported()
{
	...
	if (DeviceIoControl(g_hDevice,
		...
		&dwBytesReturned, NULL) && (dwBytesReturned > 0))
	{
		return (driverType == DT_WFP || driverType == DT_TDI);
	}
