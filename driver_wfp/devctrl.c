...
BOOLEAN	devctrl_udpNwInject()
{
	return g_udpNwInject;
}
...
NTSTATUS devctrl_init(PDRIVER_OBJECT driverObject, PUNICODE_STRING registryPath)
{
   ...
   if (NT_SUCCESS(status))
   {
	...
	for (;;)
	{
		...
		status = pFwpsInjectionHandleCreate0(
					AF_UNSPEC,
...
//	KeRemoveQueueDpc(&g_ioQueueDpc);
	...
	{
		pFwpsInjectionHandleDestroy0(g_injectionHandle);
		g_injectionHandle = NULL;
	}
	if (g_udpInjectionHandle != NULL)
	{
		pFwpsInjectionHandleDestroy0(g_udpInjectionHandle);
		g_udpInjectionHandle = NULL;
	}
	if (g_udpNwInjectionHandleV4 != NULL)
	{
		pFwpsInjectionHandleDestroy0(g_udpNwInjectionHandleV4);
		g_udpNwInjectionHandleV4 = NULL;
	}
	if (g_udpNwInjectionHandleV6 != NULL)
	{
		pFwpsInjectionHandleDestroy0(g_udpNwInjectionHandleV6);
		g_udpNwInjectionHandleV6 = NULL;
	}
	if (g_nwInjectionHandleV4 != NULL)
	{
		pFwpsInjectionHandleDestroy0(g_nwInjectionHandleV4);
		g_nwInjectionHandleV4 = NULL;
	}
	if (g_nwInjectionHandleV6 != NULL)
	{
		pFwpsInjectionHandleDestroy0(g_nwInjectionHandleV6);
...
#pragma warning(pop)
	...
	} else
	{
		if (repeat)
		{
			status = devctrl_pushTcpData(pTcpCtx->id, code, pTcpCtx, NULL);
		}
	}
   ...
   )
{
    ...
    pFwpsFreeCloneNetBufferList0(netBufferList, 0);
}
...
void devctrl_tcpInjectPackets(PTCPCTX pTcpCtx)
{
	...
	while (!IsListEmpty(&packets))
	{
		...
		{
			if ((pPacket->streamData.flags & FWPS_STREAM_FLAG_SEND) &&
				!(pPacket->streamData.flags & FWPS_STREAM_FLAG_SEND_DISCONNECT))
			{
			...
			__try {
				status = pFwpsStreamInjectAsync0(g_injectionHandle,
											 0,
											 0,
											 pTcpCtx->flowHandle,
											 pPacket->calloutId,
...
ULONG devctrl_processTcpPacket(PNF_DATA pData)
{
	...
	for (;;) 
	{
		...
		if (pData->bufferSize > 0)
		{
			...
			status = pFwpsAllocateNetBufferAndNetBufferList0(
						  g_netBufferListPool,
						  0,
						  0,
						  mdl,
	...
	if (!NT_SUCCESS(status))
	{
		...
		{
			pFwpsFreeNetBufferList0(netBufferList);
		}
		if (mdl != NULL)
		{
			IoFreeMdl(mdl);
...
ULONG devctrl_processUdpPacket(PNF_DATA pData)
{
	...
	if (!pUdpCtx)
	{
		...
		if (!pUdpCtx)
		{
			KdPrint((DPREFIX"devctrl_processUdpPacket[%I64u]: UDPCTX alloc failed\n", pData->id));
			return sizeof(NF_DATA) -1 + pData->bufferSize;
		}
	}
...
#pragma warning(pop)
		...
		if (pData->code == NF_UDP_SEND)
		{
			...
			{
				status = pFwpsConstructIpHeaderForTransportPacket0(
						  netBufferList,
						  0,
						  pUdpCtx->ip_family,
						  pLocalAddr,
			   ...
			   if (!NT_SUCCESS(status))
			   {
				...
				if (pUdpCtx->ip_family == AF_INET)
				{
					status = pFwpsInjectNetworkSendAsync0(
						   g_udpNwInjectionHandleV4,
						   NULL,
						   0,
			...
			{
				status = pFwpsInjectTransportSendAsync0(
					   g_udpInjectionHandle,
					   NULL,
		...
		} else
		{
		   ...
		   if (!NT_SUCCESS(status))
		   {
			...
			status = pFwpsInjectTransportReceiveAsync0(
					   g_udpInjectionHandle,
					   NULL,
					   NULL,
					   0,
...
{   
	...
	if (pTcpCtx->redirectInfo.isPended)
	{
		...
		if ((_memcmp(pTcpCtx->remoteAddr, pInfo->remoteAddress, addrLen) != 0) ||
			(pTcpCtx->filteringFlag & NF_BLOCK))
		{
			FWPS_CLASSIFY_OUT0 classifyOut = pTcpCtx->redirectInfo.classifyOut;
			...
			if (status == STATUS_SUCCESS)
			{
				...
				KdPrint((DPREFIX"devctrl_processTcpConnect[%I64u]: processId %d\n", pTcpCtx->id, pInfo->processId));
		...
		pTcpCtx->redirectInfo.classifyOut.rights |= FWPS_RIGHT_ACTION_WRITE;
...
{   
	...
	if (pUdpCtx->redirectInfo.isPended)
	{
		...
		if (_memcmp(pUdpCtx->remoteAddr, pInfo->remoteAddress, addrLen) != 0)
		{
			status = pFwpsAcquireWritableLayerDataPointer0(pUdpCtx->redirectInfo.classifyHandle,
			...
			if (status == STATUS_SUCCESS)
			{
				...
				pFwpsApplyModifiedLayerData0(pUdpCtx->redirectInfo.classifyHandle,
										 pConnectRequest,
										 0);
...
ULONG devctrl_processIPPacket(PNF_DATA pData)
{
	...
	for (;;) 
	{
		...
		} else
		{
			...
			status = pFwpsInjectNetworkReceiveAsync0(
					(pPacket->options.ip_family == AF_INET)? g_nwInjectionHandleV4 : g_nwInjectionHandleV6,
					(HANDLE)pPacket->options.interfaceIndex,
					0,
					(COMPARTMENT_ID)pPacket->options.compartmentId,
...
void devctrl_ioThread(IN PVOID StartContext)
{
	...
	for(;;)
	{
		...
		tcpctx_releaseHangingTCPContexts();
	}
...
#if (NTDDI_VERSION >= NTDDI_WIN8)
			if (wfplink_checkMinimumWindowsVersion(8))
				pFwpsFlowAbort0(pTcpCtx->flowHandle);
#endif
#endif
