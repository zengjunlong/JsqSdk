...
{	
	int					compartmentId;
	unsigned long long	endpointHandle;
	unsigned long		remoteScopeId;
	unsigned long		interfaceIndex;
	unsigned long		subInterfaceIndex;
	unsigned long		controlDataLength;
	unsigned long		transportHeaderLength;
...
//				if (isEventFlagEnabled(m_flags, NF_UDP_RECEIVE))
				{
					...
					while (!IsListEmpty(&pAddr->receivesIn))
					{
						...
						if (g_driverType == DT_WFP)
						{
							PNF_UDP_PACKET_OPTIONS pUdpOptions = (PNF_UDP_PACKET_OPTIONS)pOptions->options;
							memcpy(pAddr->connInfo.localAddress, pUdpOptions->localAddr, NF_MAX_ADDRESS_LENGTH);
						}
...
//				if (isEventFlagEnabled(m_flags, NF_UDP_SEND))
					...
					while (!IsListEmpty(&pConn->receivesOut))
					{
						...
						if (status == NF_STATUS_SUCCESS)
						{
							if (pDataEntry->data.bufferSize == 0)
							{
								...
								pConn->timeout = CLOSE_WAIT_TIMEOUT;
							}
	...
	public:
		...
		virtual ThreadJob * getNextJob()
		{
			...
			for (tEventList::iterator it = m_eventList.begin(); it != m_eventList.end(); )
			{
				if (it->type == ET_TCP)
				{
