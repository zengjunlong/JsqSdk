...
public:
	...
	bool setProxy(NFAPI_NS ENDPOINT_ID id, PROXY_TYPE proxyType, const char * proxyAddress, int proxyAddressLen, const char * userName = NULL, const char * userPassword = NULL)
	{
...
protected:
	...
	bool startTcpSend(PROXY_DATA * pd, bool isInSocket, const char * buf, int len, NFAPI_NS ENDPOINT_ID id)
	{
		...
		if (len != -1)
		{
			...
			psd->packetListSize += len;
		}

		if (psd->sendInProgress)
		{
			DbgPrint("TCPProxy::startTcpSend[%I64u] isInSocket=%d, send in progress", id, isInSocket);

			if (psd->packetListSize > 40 * TCP_PACKET_SIZE)
			{
				pd->suspended = true;
				DbgPrint("TCPProxy::startTcpSend[%I64u] isInSocket=%d, suspended", id, isInSocket);
			}
		...
		psd->packetListSize = 0;

		DbgPrint("TCPProxy::startTcpSend[%I64u] isInSocket=%d, resumed", id, isInSocket);
		tcpSetState(id, false);
	...
	void onReceiveComplete(SOCKET socket, DWORD dwTransferred, OV_DATA * pov, int error)
	{
		...
		if (isInSocket == OUT_SOCKET)
		{
			...
			{
				DbgPrint("TCPProxy::onReceiveComplete[%I64u] proxyState=%d", pov->id, pd->proxyState);
