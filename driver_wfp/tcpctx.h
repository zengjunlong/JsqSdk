...
#define TCP_CONTEXT_TIMEOUT (2 * 60 * 10 * 1000 * 1000)
...
typedef struct _TCPCTX
{
	...
	BOOLEAN		connectRequestPended; // TRUE if the context was created on ALE ConnectRedirect layer

	BOOLEAN		connectRequestAuthorized; // TRUE if the context was processed on ALE Authorization layer

	uint64_t	connectRequestTS;	// timestamp of the connection request
