...
void tcpctx_closeConnections()
{
	...
	while (pTcpCtx != (PTCPCTX)&g_lTcpCtx)
	{
		status = pFwpsStreamInjectAsync0(
...
void tcpctx_removeFromFlows()
{
	...
	while (!IsListEmpty(&g_lTcpCtx))
	{
		...
		status = pFwpsFlowRemoveContext0(pTcpCtx->flowHandle,
                                      pTcpCtx->layerId,
                                      pTcpCtx->sendCalloutId);
...
void tcpctx_removeContextFlows(PTCPCTX pTcpCtx)
{
	NTSTATUS status;

	KdPrint((DPREFIX"tcpctx_removeContextFlows(): TCPCTX [%I64d]\n", pTcpCtx->id));
...
void tcpctx_release(PTCPCTX pTcpCtx)
{
	...
	if (!ht_find_entry(g_phtTcpCtxById, pTcpCtx->id))
	{
		sl_unlock(&lh);
		return;
	}
...
NTSTATUS tcpctx_pushPacket(PTCPCTX pTcpCtx, FWPS_STREAM_DATA * pStreamData, int code, UINT32 calloutId)
{
	...
	// Fix the possible issue on Windows Server 2012
	if (pStreamData->dataLength & 0x8000000000000000)
	{
		pStreamData->dataLength = NET_BUFFER_DATA_LENGTH(NET_BUFFER_LIST_FIRST_NB(pStreamData->netBufferListChain));
	}
...
//		pStreamData->netBufferListChain) 
	{
		status = pFwpsCloneStreamData0(pStreamData, NULL, NULL, 0, &pPacket->streamData.netBufferListChain);
...
void tcpctx_freePacket(PNF_PACKET pPacket)
{
	...
	if (pPacket->streamData.netBufferListChain)
	{
		if (pPacket->isClone)
		{
			...
			pFwpsDiscardClonedStreamData0(pPacket->streamData.netBufferListChain, 0, isDispatch);
		} else
		{
			pFwpsFreeNetBufferList0(pPacket->streamData.netBufferListChain);
...
void tcpctx_purgeRedirectInfo(PTCPCTX pTcpCtx)
{
	...
	if (classifyHandle)
	{
		...
		{
			pFwpsCompleteClassify0(classifyHandle,
                              0,
		...
		{
			pFwpsRedirectHandleDestroy0(pTcpCtx->redirectInfo.redirectHandle);
...
void tcpctx_releaseHangingTCPContexts()
{
	KLOCK_QUEUE_HANDLE lh;
	PTCPCTX pTcpCtx, pTcpCtxDel;
 	uint64_t	ts;
 	static uint64_t	tsLast = 0;

	ts = flowctl_getTickCount();
	if ((ts - tsLast) < TCP_CONTEXT_TIMEOUT)
		return;

	tsLast = ts;

	KdPrint((DPREFIX"tcpctx_releaseHangingTCPContexts\n"));
	...
	while (pTcpCtx != (PTCPCTX)&g_lTcpCtx)
	{
		if (pTcpCtx->connectRequestPended &&
			!pTcpCtx->connectRequestAuthorized &&
			(ts - pTcpCtx->connectRequestTS) > TCP_CONTEXT_TIMEOUT &&
			!pTcpCtx->closed)
		{
			pTcpCtxDel = pTcpCtx;
			
			pTcpCtx = (PTCPCTX)pTcpCtxDel->entry.Flink;

			sl_unlock(&lh);
		
			pTcpCtxDel->closed = TRUE;

			devctrl_pushTcpData(pTcpCtxDel->id, NF_TCP_CLOSED, pTcpCtxDel, NULL);

			tcpctx_release(pTcpCtxDel);
